        ...
        match dependency_type {
            ...
            DependencyType::Dev => {
                ...
                if dev_deps.is_empty() && group_deps.is_empty() {
                    return Err(DependencyNotFoundError {
                        package: package.clone(),
                        location: "`tool.uv.dev-dependencies` or `tool.uv.dependency-groups.dev`"
                            .to_string(),
                        found_in: toml.find_dependency(&package, None),
                    }
                    .into());
            ...
            DependencyType::Optional(ref extra) => {
                ...
                if deps.is_empty() {
                    return Err(DependencyNotFoundError {
                        package: package.clone(),
                        location: format!("`project.optional-dependencies.{extra}`"),
                        found_in: toml.find_dependency(&package, None),
                    }
                    .into());
                }
                ...
                if group == &*DEV_DEPENDENCIES {
                    ...
                    if dev_deps.is_empty() && group_deps.is_empty() {
                        return Err(DependencyNotFoundError {
                            package: package.clone(),
                            location:
                                "`tool.uv.dev-dependencies` or `tool.uv.dependency-groups.dev`"
                                    .to_string(),
                            found_in: toml.find_dependency(&package, None),
                        }
                        .into());
                    ...
                    if deps.is_empty() {
                        return Err(DependencyNotFoundError {
                            package: package.clone(),
                            location: format!("`dependency-groups.{group}`"),
                            found_in: toml.find_dependency(&package, None),
                        }
                        .into());
                    }
...
#[error("The dependency `{package}` could not be found in {location}")]
